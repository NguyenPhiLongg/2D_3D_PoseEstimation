...
#   -> convert YOLOv8 COCO17 keypoints to paper-style skeleton
#   -> remove confidence score
#   -> relative position normalization
#   -> missing-value interpolation
#   -> RNN / LSTM / GRU
#
# This file corresponds to the skeleton extraction stage.
# It does NOT train a model.
# It does NOT normalize skeletons.
# It does NOT build sequences.
#
# It only collects existing YOLOv8 keypoints from the project and stores them in a clean
# per-video CSV format:
#
#   video_id, frame_idx, label, label_name, has_person,
#   joint_0_x, joint_0_y, joint_0_c,
#   ...
#   joint_16_x, joint_16_y, joint_16_c
#
# YOLOv8 uses COCO17 keypoints:
#   0 nose
#   1 left_eye
#   2 right_eye
#   3 left_ear
...
#   16 right_ankle
#
# File 02 will convert COCO17 -> paper-style 15 joints.
# ======================================================================================
...
def build_flat_keypoint_map(
    ...
    """
    Fallback for files where columns are flattened numerically.

    Supported layouts after removing metadata:
      - 51 numeric columns: x0,y0,c0, x1,y1,c1, ..., x16,y16,c16
      - 34 numeric columns: x0,y0, x1,y1, ..., x16,y16
    """
...
def collect_candidate_csv_files(project_root: Path, input_path: Optional[str]) -> List[Path]:
    ...
    # Avoid old paper baseline outputs if any.
    filtered = []
...
def infer_selected_root(selected_files: List[Path]) -> Path:
    ...
    priorities = [source_priority(p) for p in selected_files]
    best = min(priorities)
...
# ======================================================================================
# CONVERSION TO RAW YOLOV8 COCO17 CSV
# ======================================================================================
...
def clean_video_text(value) -> str:
    ...
    # If the value contains the raw video root, keep only the relative part.
...
def normalize_video_id(
    ...
    # If this comes from a per-video CSV and no video column exists,
    # use the relative CSV path to avoid duplicate names.
...
def build_raw_yolov8_dataframe(
    ...
    if label_col is not None:
        labels = df[label_col].apply(
            lambda v: parse_label_value(v, fallback_text=source_text)
        )
    else:
    ...
    # Add label prefix to avoid collision between Fall/B_D_0001 and Not_Fall/B_D_0001.
    ...
    for i in range(17):
        ...
        if mapping[i]["c"] is not None:
            ...
            # If the original file has no confidence, treat non-zero coordinate as detected.
    ...
    # At least 3 valid joints means the frame has some detected skeleton signal.
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect existing YOLOv8-Pose keypoints for Lin-style RNN/LSTM/GRU baseline."
    )
    ...
    parser.add_argument(
        "--use-all-usable",
        action="store_true",
        help="Use all usable CSV files. By default, the script chooses the best source priority to avoid duplicates.",
    )
    ...
    report = {
        "status": "completed",
        "pipeline_note": "Adapted Lin et al. 2021-style pipeline. YOLOv8-Pose replaces OpenPose only at the keypoint extraction stage. Later steps still follow skeleton preprocessing and RNN/LSTM/GRU temporal classification.",
