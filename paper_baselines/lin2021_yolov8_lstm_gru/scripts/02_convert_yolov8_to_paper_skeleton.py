...
# ======================================================================================
# FILE 02: CONVERT YOLOV8 COCO17 TO PAPER-STYLE 15-JOINT SKELETON
# ======================================================================================
#
# Original Lin et al. 2021-style pipeline:
#   OpenPose keypoints
#   -> skeleton preprocessing
#   -> RNN / LSTM / GRU
#
# Adapted pipeline in this project:
#   YOLOv8-Pose COCO17 keypoints
#   -> convert to paper-style 15-joint skeleton
#   -> skeleton preprocessing
#   -> RNN / LSTM / GRU
#
# This file does NOT normalize.
# This file does NOT remove confidence yet.
# This file only maps YOLOv8 COCO17 joints to a 15-joint skeleton format.
#
# Input from File 01:
...
#
# Paper-style 15 joints used here:
#   0  nose
#   1  neck = average(left_shoulder, right_shoulder)
#   2  right_shoulder
#   3  right_elbow
#   4  right_wrist
#   5  left_shoulder
#   6  left_elbow
#   7  left_wrist
#   8  mid_hip = average(left_hip, right_hip)
#   9  right_hip
#   10 right_knee
#   11 right_ankle
#   12 left_hip
...
def validate_raw_yolov8_df(df: pd.DataFrame, csv_path: Path):
    ...
    if missing:
        raise ValueError(
            f"Missing required YOLOv8 raw columns in {csv_path}. Missing examples: {missing[:20]}"
        )
...
def coco17_to_paper15(row: pd.Series) -> np.ndarray:
    """
    Convert one frame from YOLOv8 COCO17 format to paper-style 15-joint format.
    """
    ...
    neck = average_points(left_shoulder, right_shoulder)
    mid_hip = average_points(left_hip, right_hip)

    paper15 = np.zeros((15, 3), dtype=np.float32)

    paper15[0] = nose
    paper15[1] = neck

    paper15[2] = right_shoulder
    paper15[3] = right_elbow
    paper15[4] = right_wrist

    paper15[5] = left_shoulder
    paper15[6] = left_elbow
    paper15[7] = left_wrist

    paper15[8] = mid_hip

    paper15[9] = right_hip
    paper15[10] = right_knee
    paper15[11] = right_ankle

    paper15[12] = left_hip
    paper15[13] = left_knee
    paper15[14] = left_ankle
...
def convert_one_dataframe(df: pd.DataFrame, csv_path: Path) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        for j in range(15):
            ...
            out_row[f"joint_{j}_name"] = PAPER15_NAMES[j]
            out_row[f"joint_{j}_x"] = x
            out_row[f"joint_{j}_y"] = y
            out_row[f"joint_{j}_c"] = c

            if c > 0.0 and (x != 0.0 or y != 0.0):
                valid_joint_count += 1

        out_row["valid_joint_count"] = int(valid_joint_count)
        out_row["has_person"] = int(valid_joint_count >= 3)
    ...
    out_df = out_df[meta_cols + joint_cols]
...
def process_one_csv(csv_path: Path, output_dir: Path) -> Dict:
    ...
    result = {
        ...
        "mean_valid_joint_count": float(out_df["valid_joint_count"].mean()),
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert raw YOLOv8 COCO17 keypoints to paper-style 15-joint skeleton CSV."
    )
    ...
    parser.add_argument(
        ...
        help="Output folder for paper-style 15-joint skeleton CSV files.",
    )
    ...
    print("=" * 100)
    print("FILE 02 - Convert YOLOv8 COCO17 to paper-style 15-joint skeleton")
    print("=" * 100)
    ...
    if len(csv_files) == 0:
        ...
        report_path = reports_dir / "02_convert_yolov8_to_paper_skeleton_report.json"
    ...
    for idx, csv_path in enumerate(csv_files, start=1):
        ...
        try:
            ...
            print(
                ...
                f"mean_valid_joints={result['mean_valid_joint_count']:.2f}"
            )
    ...
    index_path = output_dir / "all_paper_skeleton_csv_index.csv"
    ...
    report = {
        "status": "completed",
        "pipeline_note": "YOLOv8 COCO17 keypoints were converted to a paper-style 15-joint skeleton. Confidence is still kept here. File 03 will remove confidence and apply RP-normalization plus interpolation.",
        "input_dir": str(input_dir),
        ...
        "paper15_joint_names": PAPER15_NAMES,
