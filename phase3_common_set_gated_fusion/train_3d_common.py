"""
Train 3D Common-set model for Phase 3.

Purpose:
- Train the 3D-only model on the SAME common video set used by 2D,
  concat fusion, and gated fusion.
- This makes the comparison fair.

Input:
    Estimated 3D features only
    Shape per sequence: (60, 59)

Output:
    Binary:
        outputs/training_3d_common/results_3d_common_binary.json
        checkpoints/best_model_3d_common_binary.pt

    Action:
        outputs/training_3d_common/results_3d_common_action.json
        checkpoints/best_model_3d_common_action.pt
...
def get_output_paths(task: str) -> tuple[Path, Path]:
    ...
    output_dir = OUTPUT_DIR / "training_3d_common"
    output_dir.mkdir(parents=True, exist_ok=True)

    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)

    if task == "binary":
        output_json = output_dir / "results_3d_common_binary.json"
        checkpoint = CHECKPOINT_DIR / "best_model_3d_common_binary.pt"

    elif task == "action":
        output_json = output_dir / "results_3d_common_action.json"
        checkpoint = CHECKPOINT_DIR / "best_model_3d_common_action.pt"
...
def build_model(
    ...
    """
    Build 3D CNN1D + BiLSTM model.

    The architecture follows the Phase 2 3D model.
    The dataset is restricted to the Phase 3 common set.

    3D input:
        51 normalized 3D coordinates
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train 3D-only CNN1D-BiLSTM model on Phase 3 common set."
    )
...
def main() -> None:
    ...
    print("=" * 80)
    print("PHASE 3 - TRAIN 3D COMMON-SET MODEL")
    print("=" * 80)
    print("Task:", task)
    print("Feature mode: 3d")
    print("Input dimension: 59D")
    ...
    train_single_input_model(
        ...
        model_type="CNN1D_BiLSTM_3D_COMMON",
        input_dim=59,
